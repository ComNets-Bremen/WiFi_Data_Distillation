...
from save_syn_to_excel import save_image_syn_eval_to_excel
import pandas as pd
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                for it_eval in range(args.num_eval):
                    ...
                    print(f"image_syn_eval → shape: {image_syn_eval.shape} | dtype: {image_syn_eval.dtype} | min: {image_syn_eval.min():.4f} | max {image_syn_eval.max():.4f}")
                    print(f"label_syn_eval → shape: {label_syn_eval.shape} | dtype: {label_syn_eval.dtype} | unique labels: {label_syn_eval.unique()}")
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                run_name = wandb.run.name if (wandb.run and wandb.run.name) else "offline_run"
                ...
                if args.ipc < 0 or args.force_save:
                    ...
                    wandb.log({"Synthetic": wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                    wandb.log({'Synthetic': wandb.Histogram(torch.nan_to_num(image_save.detach().cpu()))}, step=it)
        ...
        # -----------------------------
        old_syn_images = image_syn.detach().cpu().clone()

        grand_loss.backward()
        grad_norm = image_syn.grad.norm().item()
        print(f"image_syn grad norm: {grad_norm:.6e}")
