...
import torch
# Acknowledgement to
#https://github.com/xyanchen/WiFi-CSI-Sensing-Benchmark/blob/main/widar_model.py
...
class Widar_LSTM(nn.Module):
    def __init__(self,num_classes):
        ...
        self.fc = nn.Linear(64,num_classes)
    def forward(self, x):
        # Reshape and transpose the input tensor x
        x = x.view(-1, 22, 400)
        x = x.permute(1, 0, 2)

        # Disable CuDNN for LSTM forward and backward propagation
        with torch.backends.cudnn.flags(enabled=False):
            _, (ht, ct) = self.lstm(x)
        
        # Apply fully connected layer computation to produce final output
        outputs = self.fc(ht[-1])
        return outputs
